...
export default function ProductCard() {
    ...
    return (
        <div className='productcard'>
            <ul className="productcard__list list-reset">
                {products.map((product) => {
                    const discountNum = Number(product.discount) || 0;
                    const priceNum = Number(product.price) || 0;
                    const originalPrice = discountNum > 0 ? (priceNum / (1 - discountNum / 100)).toFixed(2) : null;

                    return (
                        <li key={product.id} className="productcard__item">
                            <div className="productcard__image-container">
                                {discountNum > 0 && (
                                    <span className="productcard__badge">
                                        {discountNum}% OFF
                                    </span>
                                )}
                                <img
                            ...
                            <h3 className="productcard__subtitle">
                                {product.name}
                            </h3>
